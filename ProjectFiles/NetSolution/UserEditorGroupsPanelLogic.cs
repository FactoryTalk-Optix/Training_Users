...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, Training_Users.ObjectTypes.GroupCheckbox);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, Training_Users.ObjectTypes.GroupLabel);
